function bddStdin(responses) {
  ...
  if (!Array.isArray(responses)) {
    throw new Error(
      'Expected array of responses, not ' + JSON.stringify(responses, null, 2)
    )
  }
  if (responses.length < 1) {
    throw new Error(
      'Expected at least 1 response, not ' + JSON.stringify(responses, null, 2)
    )
  }
  ...
  function sendAnswer() {
    setTimeout(function () {
      ...
      if (typeof text !== 'string') {
        throw new Error(
          'Should give only text responses ' +
